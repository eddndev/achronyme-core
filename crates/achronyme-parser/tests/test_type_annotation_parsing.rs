...
fn test_lambda_with_function_return_type() {
    let result = parse("(): Function<[Number], Number> => (x: Number) => x^2");
    assert!(result.is_ok());
...
fn test_higher_order_function_with_types() {
    let result = parse("let map: Function<[Function<[Number], Number>, Vector], Vector> = (f: Function<[Number], Number>, v) => v");
    assert!(result.is_ok());
        ...
        AstNode::VariableDecl { name, type_annotation, .. } => {
            ...
            match type_annotation {
                Some(TypeAnnotation::Function { params, return_type }) => {
                    ...
                    // First param: Function<[Number], Number>
                    match &params[0] {
